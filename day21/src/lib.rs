...
impl<'a> Foods<'a> {
    fn new() -> Foods<'a> { Default::default() }
    ...
    fn deduce_ingredients_map(&mut self) -> bool {
        ...
        for allergen in self.all_allergens.iter() {
            ...
            let known_ingredients: HashSet<&'a str> = self.ingredients_map.keys().map(|x| *x).collect();
            possible_ingredients = possible_ingredients.difference(&known_ingredients).map(|x| *x).collect();
            ...
            if possible_ingredients.len() == 1 {
                let ingredients = possible_ingredients.iter().map(|x| *x).collect::<Vec<&'a str>>();
...
pub fn part1() -> usize {
    ...
    let known_ingredients: HashSet<&str> = foods.ingredients_map.keys()
        .map(|x| *x).collect();
    let unknown_ingredients: HashSet<&str> = foods.all_ingredients.difference(&known_ingredients)
        .map(|x| *x).collect();
    ...
    let mut sorted_allergens: Vec<&str> = foods.allergens_map.keys()
        .map(|x| *x).collect();
