...
fn flip_edge(edge: u16) -> u16 {
    return (0..=9)
        .map(|x| if (edge & (1 << x)) != 0 { 1 << (9-x) } else { 0 })
        .sum();
}
...
impl Tile {
    ...
    fn is_corner(&self) -> bool {
        let count = self.connections.iter()
            .filter(|x| x.is_none())
            .count();
        return count == 2;
    }
    ...
    fn get_bit(&self, x: usize, y: usize) -> i8 {
        ...
        return if (self.bitmap[iy] & (1 << (7-ix))) == 0 { 0 } else { 1 };
    }

    fn dump(&self) {
        println!("tile:{} connections:N:{:?},E:{:?},S:{:?},W:{:?} flipped:{} rotated:{}",
            self.tile_id,
            self.connections[0],
            self.connections[1],
            self.connections[2],
...
impl TileSet {
    ...
    fn find_matching_edges(&self, edge_id: EdgeId) -> Vec<EdgeId> {
        ...
        let edge_value = self.tilemap[&edge_id.tile_id]
            .edges[edge_id.dir];
        let edge_flipped = flip_edge(edge_value);

        for tile in self.tilemap.values() {
            // Don't match against ourselves:
            if tile.tile_id == edge_id.tile_id { continue; }
    ...
    fn connect_edges(&mut self, e1: EdgeId, e2: EdgeId) {
        ...
        self.tilemap.get_mut(&e1.tile_id).unwrap()
            .connections[e1.dir] = Option::Some(e2);
        self.tilemap.get_mut(&e2.tile_id).unwrap()
            .connections[e2.dir] = Option::Some(e1);
    }

    pub fn connect(&mut self) {
        let mut v: Vec<&Tile> = self.tilemap.values().collect();
        v.sort();  // or, v.sort_by_key(|x| x.tile_id);
        let connections: Vec<(EdgeId, Vec<EdgeId>)> = v.iter()
              .flat_map(|tile| EDGES.iter()
                  .map(move |dir| EdgeId::new(tile, *dir, false)))
              .map(|e| (e, self.find_matching_edges(e)))
              .collect();

        let unique_connections: Vec<(EdgeId, EdgeId)> = connections.iter()
              .filter(|(_, m)| m.len() == 1)
              .map(|(e, m)| (*e, m[0]))
              .collect();
    ...
    fn get_corners(&self) -> Vec<u16> {
        let mut corners: Vec<u16> = self.tilemap.values()
    ...
    fn place_tile(&mut self, x: usize, y: usize) {
        ...
        println!("PLACING from tile {},{} ({}) to tile {},{} via {}",
            adj_x, adj_y, adj_tile_id, x, y, dir_to_name(dir_from_adj));
        print!("  adj_tile: ");
        adj_tile.dump();
        println!("  adj_tile {} maps to {}: con={:?}",
        ...
        let conn = adj_tile.connections[
            adj_tile.transform_dir(dir_from_adj)].unwrap();
        ...
        if conn.flipped ^ adj_tile.flipped { tile.flip(); }
        // Rotate the tile until edge the previous tile connected to is
        // facing the previous tile.
        while tile.transform_dir(dir_to_adj) != conn.dir { tile.rotate(); }
        print!("  placed: ");
        tile.dump();
    }
    ...
    pub fn layout(&mut self, i: usize) {
        ...
        for y in 0..(self.n-1) as usize {
            for x in 0..(self.n-1) as usize {
                ...
                assert_eq!(tile.get_xformed_edge(E), flip_edge(tile_e.get_xformed_edge(W)));
                assert_eq!(tile.get_xformed_edge(S), flip_edge(tile_s.get_xformed_edge(N)));
            }
        }
    }
...
pub static SEAMONSTER: [Point; 15] = [
  (0,1), (1,2), (4,2), (5,1), (6, 1), (7, 2),  // tail, 1st hump
  (10,2), (11,1), (12,1), (13,2),  // 2nd hump
  (16,2), (17,1), (18,1), (18,0), (19,1),  // head
];
    ...
    pub fn dump(&self) {
        for y in 0..self.n {
            for x in (0..self.n).step_by(8) {
                ...
                let chrs: String = self.pixels[index..(index+8)].iter()
