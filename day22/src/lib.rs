...
fn deck_to_str(deck: &Deck) -> String {
    let tmp: Vec<String> = deck.iter()
        .map(|x| x.to_string() as String)
        .collect();
    return tmp.join(", ");
}
...
impl Game {
    ...
    fn parse_deck(&mut self, deck_index: usize, text: &str) {
        self.decks[deck_index] = text.lines()
    ...
    fn do_round(&mut self, game_number: usize, mode: Mode) -> bool {
        ...
        let cards: Vec<Card>  = self.decks.iter_mut().map(|deck| deck.pop_front().unwrap()).collect();

        if mode == Mode::Part2 {
            if (self.decks[0].len() >= cards[0] as usize) && (self.decks[1].len() >= cards[1] as usize) {
    ...
    fn do_game(&mut self, game_number: usize, mode: Mode) {
        ...
        while self.do_round(game_number, mode) {
            if history.contains(self) {
                ...
                self.decks[1].clear();
                debug!("game {} done, repeat state winner={}", game_number, self.get_winner());
                return;
            }
    ...
    fn score(&self) -> i64 {
        let winner: &Deck = if self.decks[0].len() == 0 { &self.decks[1] } else { &self.decks[0] };
        let score: i64 = winner.iter().rev().enumerate()
