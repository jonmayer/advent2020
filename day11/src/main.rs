...
impl SeatMap {
    ...
    fn run(&mut self, part2: bool) -> u32 {
        let mut steps = 0u32;
        while self.run_one_step(part2) { steps += 1 };
        return steps;
    }
...
fn visibility_test(text: &str, row: i32, col: i32, visible: u8) {
    ...
    assert_eq!(seatmap_visibility_test1.count_visible_occupants(row, col), visible);
...
fn run_test(text: &str, final_count: u32) {
    ...
    loop {
        ...
        println!("changed={} count={}", changed, seatmap.count_all_occupants());
        if !changed { break };
    }
